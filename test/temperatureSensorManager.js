...
describe("TemperatureSensorManager", function () {
  ...
  beforeEach(async function () {
    ...
    HumiditySensorManager = await ethers.getContractFactory("TemperatureSensorManager");
  ...
  it("Registra um sensor de umidade corretamente", async function () {
    await humiditySensorManager.registerHumiditySensor("UID789", "BB:11:22:33:44:55");
    const sensor = await humiditySensorManager.sensors("UID789");
  ...
  it("Falha ao tentar registrar o mesmo UID novamente", async function () {
    await humiditySensorManager.registerHumiditySensor("UID789", "BB:11:22:33:44:55");
    await expect(
      humiditySensorManager.registerHumiditySensor("UID789", "AA:BB:CC:DD:EE:FF")
    ).to.be.revertedWith("Device already registered");
  });

  it("Autentica um sensor válido dentro do prazo", async function () {
    await humiditySensorManager.registerHumiditySensor("UID789", "BB:11:22:33:44:55");
    const isAuthentic = await humiditySensorManager.isHumiditySensorAuthentic("UID789");
  ...
  it("Impede que contas não-admin registrem sensores", async function () {
    await expect(
      humiditySensorManager.connect(otherAccount).registerHumiditySensor("UID789", "11:22:33:44:55:66")
